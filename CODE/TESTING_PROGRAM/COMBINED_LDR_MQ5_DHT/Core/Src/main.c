...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  LdrData = HAL_ADC_GetValue(&hadc1);
	  HAL_ADC_Stop(&hadc1) ;

	  HAL_ADC_Start(&hadc2);
	  HAL_ADC_PollForConversion(&hadc2, HAL_MAX_DELAY);
	  MQ5Data = HAL_ADC_GetValue(&hadc2);
	  HAL_ADC_Stop(&hadc2) ;
	  ...
	  uint16_t raw_humidity = (Rh_byte1 << 8) | Rh_byte2;
	  uint16_t raw_temp = (Temp_byte1 << 8) | Temp_byte2;
	  uint8_t check = Rh_byte1 + Rh_byte2 + Temp_byte1 + Temp_byte2;

	  if (!(check == SUM))
	  {
		  sprintf(str, "Checksum error!\r\n");
	  }
	  else
	  {
		  Humidity = raw_humidity / 10.0;
		  Temperature = raw_temp / 10.0;
		  sprintf(str, "Temp: %.1f C, RH: %.1f %%   LDR: %2d    GAS: %2d \r\n", Temperature, Humidity, LdrData , MQ5Data);
	  }
